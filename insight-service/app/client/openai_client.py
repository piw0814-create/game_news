...
- For an unnamed future entry, do NOT create/return a Game named only after the franchise. Instead set the article entityType to UNNAMED_ENTRY and, when the franchise exists in Known franchises, return it in relatedFranchises with entityType=UNNAMED_ENTRY.
...
- Only return franchises present in Known franchises. Never invent or auto-create a franchise from the article.
...
- Example: "The GTA franchise surpassed 500 million sales." -> entityType=FRANCHISE; relatedGames=[]; relatedFranchises may contain Grand Theft Auto with entityType=FRANCHISE if it exists in Known franchises.
...
- Example: "The next Mass Effect game is still years away." -> entityType=UNNAMED_ENTRY; relatedGames=[]; if Mass Effect exists in Known franchises, relatedFranchises=[Mass Effect with entityType=UNNAMED_ENTRY]. Do NOT return/create the specific game Mass Effect merely because that franchise name appears.
