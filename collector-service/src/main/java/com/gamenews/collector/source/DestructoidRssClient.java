...
public class DestructoidRssClient {

    private static final String SOURCE_NAME = "Destructoid";
    private static final String USER_AGENT = "game-news-collector/1.0";

    private final WebClient.Builder webClientBuilder;

    @Value("${collector.sources.destructoid.rss-url}")
    ...
    public List<RssArticle> fetchLatest(int limit) {
        try {
            ...
            if (rssBytes == null || rssBytes.length == 0) {
                throw new IllegalStateException("Destructoid RSS 응답이 비어 있습니다");
            }
            ...
            throw new IllegalStateException("Destructoid RSS 수집에 실패했습니다: " + e.getMessage(), e);
        }
    }
