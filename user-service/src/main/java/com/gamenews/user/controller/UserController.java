...
import com.gamenews.user.service.UserService;
...
public class UserController {
    ...
    public ResponseEntity<UserDto.ApiResponse<UserDto.PublicUserResponse>> register(
            @Valid @RequestBody UserDto.RegisterRequest request) {
        UserDto.UserResponse response = userService.register(request);
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(UserDto.ApiResponse.success(UserDto.PublicUserResponse.from(response)));
    }
     ...
     */
    @GetMapping("/me")
    public ResponseEntity<UserDto.ApiResponse<UserDto.PublicUserResponse>> getMe(
            @RequestHeader("X-User-Id") Long userId) {

        UserDto.UserResponse response = userService.getUserById(userId);
        return ResponseEntity.ok(UserDto.ApiResponse.success(UserDto.PublicUserResponse.from(response)));
    }
